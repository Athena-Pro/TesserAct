...
const V4_LERP = (a: V4, b: V4, t: number): V4 => ({
  x: a.x + (b.x - a.x) * t,
  y: a.y + (b.y - a.y) * t,
  z: a.z + (b.z - a.z) * t,
  w: a.w + (b.w - a.w) * t,
...
const ROT_K = 0.005;
function inferPublicIntents(
  dx: number,
  dy: number,
  wMode: boolean,
  keySpec: DimensionalKeySpec,
): { plane: Plane; angle: number }[] {
  ...
  const ax = Math.abs(dx),
    ay = Math.abs(dy);
  if (ax > ay * 1.2 && wPublic.includes('XW'))
    return [{ plane: 'XW', angle: dx * ROT_K }];
  if (ay > ax * 1.2 && wPublic.includes('YW'))
    return [{ plane: 'YW', angle: dy * ROT_K }];
  ...
  return [
    {
      plane: wPublic[0],
      angle: Math.hypot(dx, dy) * ROT_K * (dx >= 0 ? 1 : -1),
    },
  ];
}
